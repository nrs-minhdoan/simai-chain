...
| `transaction/processor.ts`   | Luật chuyển trạng thái transfer/deploy/call + `query` (read-only). | Thứ tự: chữ ký → nonce → số dư → thực thi.      |
| `block/block.ts`             | Header, hash block, `txRoot`.                                      | Header cam kết `txRoot`+`stateRoot`+`prevHash`. |
| `consensus/consensus.ts`     | BFT Tendermint-style + **telemetry** cho UI.                       | Ngưỡng `power*3 > total*2`.                     |
...
**Đổi số validator / quyền biểu quyết:** sửa trong `ChainSimulator.reset()`
(`explorer/src/simulator/chain-simulator.ts`) và/hoặc `core/demo.ts`. Ngưỡng tự tính theo
tổng `power`.
