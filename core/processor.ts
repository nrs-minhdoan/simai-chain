...
/** Địa chỉ contract khi deploy = hash(from || nonce), tất định. */
function contractAddress(from: Hex, nonce: bigint): Hex {
  const b = new Uint8Array(28);
  b.set(fromHex(from), 0);
  const nb = nonce.toString(16).padStart(16, '0');
  b.set(new Uint8Array(Buffer.from(nb, 'hex')), 20);
  return toHex(hash256(b).slice(-20));
}

export function applyTx(state: WorldState, tx: Tx): ApplyResult {
  if (!verifyTx(tx)) return { ok: false, error: 'chữ ký không hợp lệ / tx bị sửa' };

  const sender = state.get(tx.from);
  if (tx.nonce !== sender.nonce)
    return { ok: false, error: `nonce sai (mong đợi ${sender.nonce}, nhận ${tx.nonce}) — có thể là replay` };

  if (tx.type === 'transfer') {
    if (!tx.to) return { ok: false, error: 'thiếu địa chỉ đích' };
    if (sender.balance < tx.value) return { ok: false, error: 'không đủ số dư' };
  ...
  if (tx.type === 'deploy') {
    ...
    const addr = contractAddress(tx.from, tx.nonce);
    state.get(addr).code = tx.code;
    sender.nonce += 1n;
  ...
  if (sender.balance < tx.value) return { ok: false, error: 'không đủ số dư để gửi value' };
  ...
  return { ok: false, kind: 'call-revert', error: 'revert: ' + result.reason, gasUsed: result.gasUsed };
}

/** Đọc read-only (không đổi state, không cần nonce/chữ ký) — như eth_call. */
export function query(state: WorldState, contractAddr: Hex, args: bigint[], gasLimit = 100000n): bigint | null {
  const acct = state.get(contractAddr);
  if (!acct.code) return null;
  const r = run(acct.code, { caller: 0n, value: 0n, args, gasLimit, storage: acct.storage });
