...
import { makeTx, verifyTx, txDigest } from "./src/transaction/transaction";
import { applyTx, query } from "./src/transaction/processor";
...
line(
  `  BigInt fixed-point: 0.1 + 0.2 = ${formatFixed(parseFixed("0.1") + parseFixed("0.2"))}   ✅`,
);
const huge = parseFixed("9007199254740993.000000000000000001");
line(
  `  BigInt fixed-point: số rất lớn+lẻ = ${formatFixed(huge)}   ✅ chính xác tuyệt đối`,
);
...
validators.forEach((v) =>
  line(`  validator ${v.name}: ${short(v.wallet.address)}`),
);
line(
  `  alice: ${short(alice.address)}   bob: ${short(bob.address)}   carol: ${short(carol.address)}`,
);
...
const byzV: Validator[] = validators.map((v, i) => ({
  ...v,
  byzantine: i === 3 ? "equivocate" : "none",
}));
chain.commit([tx1, tx2], { validators: byzV, log: line });
line(
  `  verifyCommit(bên thứ ba) = ${verifyCommit(chain.head!, validators, 1)}`,
);
line(
  `  Số dư: alice=${formatFixed(chain.state.balanceOf(alice.address))}, bob=${formatFixed(chain.state.balanceOf(bob.address))}, carol=${formatFixed(chain.state.balanceOf(carol.address))}`,
);
...
const balA = query(chain.state, tokenAddr, [
  M_BALANCEOF,
  addrToBig(alice.address),
]);
const balB = query(chain.state, tokenAddr, [
  M_BALANCEOF,
  addrToBig(bob.address),
]);
line(
  `  token.balanceOf(alice) = ${formatFixed(balA!)}   token.balanceOf(bob) = ${formatFixed(balB!)}`,
);
...
function replayTo(heightExclusive: number): WorldState {
  ...
  for (let h = 1; h < heightExclusive; h++)
    for (const tx of chain.blocks[h]!.txs) applyTx(s, tx);
  return s;
}
...
line(
  `  4 validator tự tính stateRoot -> trùng nhau? ${new Set(roots).size === 1 ? "✅ CÓ (tất định)" : "❌ KHÔNG"}`,
);
...
line(
  `  6a REPLAY tx cũ            -> ${liveReplay.ok ? "CHẤP NHẬN ❌" : "TỪ CHỐI ✅"}`,
);
const forged: Tx = { ...tx1, value: parseFixed("999999") };
line(
  `  6b GIẢ MẠO value           -> verifyTx = ${verifyTx(forged) ? "HỢP LỆ ❌" : "KHÔNG HỢP LỆ ✅"}`,
);
...
line(
  `  6c SỬA block đã chốt       -> verifyCommit = ${verifyCommit(tampered, validators, 1) ? "TRUE ❌" : "FALSE ✅"}`,
);
const chain2 = new Chain(validators);
chain2.state.credit(alice.address, parseFixed("100"));
chain2.commit([]);
const halfSilent: Validator[] = validators.map((v, i) => ({
  ...v,
  byzantine: i >= 2 ? "silent" : "none",
}));
...
line(
  `  6d NGƯỠNG 1/3 (2/4 im lặng) -> ${rr.committed ? "CHỐT ❌" : "KHÔNG CHỐT ✅ (dừng an toàn)"}`,
);
const evilMint = makeTx({
  type: "call",
  from: bob.address,
  priv: bob.priv,
  nonce: 1n,
...
const balBAfter = query(chain.state, tokenAddr, [
  M_BALANCEOF,
  addrToBig(bob.address),
]);
line(
  `  6e MINT trái phép (bob)     -> balanceOf(bob) vẫn = ${formatFixed(balBAfter!)} ✅`,
);

H("KẾT LUẬN");
line(
  `  Chain cao ${chain.height} block, mọi bất biến ✅. Xem trực quan bằng explorer: pnpm dev`,
);
