...
import { hash256, sign, recoverAddress, fromHex, toHex } from './crypto';
import { applyTx } from './processor';
import { makeBlock, txRootOf } from './block';
import { WorldState } from './state';
import type { Validator, Vote, VoteStep, Block, Tx, Wallet, Hex, RoundTelemetry, VoteInfo } from './types';

function voteDigest(height: number, round: number, step: VoteStep, blockHash: Hex): Uint8Array {
  return hash256(encode([BigInt(height), BigInt(round), step, fromHex(blockHash)]));
}

function signVote(w: Wallet, height: number, round: number, step: VoteStep, blockHash: Hex): Vote {
  const { compact, recovery } = sign(voteDigest(height, round, step, blockHash), w.priv);
  return { voter: w.address, step, blockHash, sig: { compact, recovery } };
}

function verifyVote(v: Vote, height: number, round: number, set: Set<Hex>): boolean {
  try {
...
function tally(votes: Vote[], target: Hex, powers: Map<Hex, bigint>, height: number, round: number, set: Set<Hex>): bigint {
  let power = 0n;
...
export function proposeBlock(
  proposer: Wallet, height: number, prevHash: Hex, timestamp: bigint, preState: WorldState, mempool: Tx[],
): { block: Block; postState: WorldState } {
  const working = preState.clone();
  const included: Tx[] = [];
  for (const tx of mempool) {
    const r = applyTx(working, tx);
    if (r.ok || r.kind === 'call-revert') included.push(tx); // tx sai chữ ký/nonce bị loại
  }
  const block = makeBlock({
    height: BigInt(height), prevHash, txs: included,
    stateRoot: working.stateRoot(), proposer: proposer.address, timestamp,
  });
  return { block, postState: working };
}
...
export type RunRoundResult =
  | { committed: true; block: Block; postState: WorldState; height: number; round: number; telemetry: RoundTelemetry }
...
function voteInfoFor(v: Validator, blockValid: boolean): 'valid' | 'bogus' | 'nil' {
...
export function runRound(p: RunRoundParams): RunRoundResult {
  ...
  const powers = new Map<Hex, bigint>(validators.map((v) => [v.wallet.address, v.power]));
  ...
  log(`  height ${height} round ${round}: proposer = ${short(proposer.wallet.address)}`);

  const { block, postState } = proposeBlock(proposer.wallet, height, prevHash, BigInt(height), preState, mempool);
  ...
  const voteInfos: VoteInfo[] = validators.map((v) => ({
    voter: v.wallet.address, name: v.name, power: v.power, voted: voteInfoFor(v, blockValid),
  }));
  ...
  for (const v of validators) {
    ...
    const target = v.byzantine === 'equivocate' ? bogus : blockValid ? good : null;
    if (target) prevotes.push(signVote(v.wallet, height, round, 'prevote', target));
  }
  const prevotePower = tally(prevotes, good, powers, height, round, set);
  log(`  PRE-VOTE  cho block hợp lệ: ${prevotePower}/${totalPower} quyền biểu quyết  (cần > ${(totalPower * 2n) / 3n})`);
  ...
  const telemetry: RoundTelemetry = {
    height, round, proposer: proposer.wallet.address, totalPower, thresholdPower,
    goodHash: good, prevotes: voteInfos, precommits: voteInfos,
    prevotePower, precommitPower, committed: false,
  };

  if (!over2_3(prevotePower, totalPower)) {
    log(`  ✗ không đạt > 2/3 pre-vote -> KHÔNG chốt (an toàn: không có block xung đột nào commit)`);
    return { committed: false, reason: 'no-polka', telemetry };
  }
  log(`  PRE-COMMIT cho block hợp lệ: ${precommitPower}/${totalPower} quyền biểu quyết  (cần > ${(totalPower * 2n) / 3n})`);
  ...
  block.commit = precommits.filter((pc) => pc.blockHash === good && verifyVote(pc, height, round, set));
  log(`  ✓ COMMIT block ${short(block.hash)} với ${block.commit.length} chữ ký pre-commit`);
  return { committed: true, block, postState, height, round, telemetry: { ...telemetry, committed: true } };
}

/** Kiểm tra bằng chứng chốt của một block (bên thứ ba xác minh được). */
export function verifyCommit(block: Block, validators: Validator[], height: number, round = 0): boolean {
  const powers = new Map<Hex, bigint>(validators.map((v) => [v.wallet.address, v.power]));
  const set = new Set<Hex>(powers.keys());
