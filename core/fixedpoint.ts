// ---------------------------------------------------------------------------
// fixedpoint.ts — Xử lý số CHÍNH XÁC. Mọi giá trị là integer BigInt đã nhân
...
export function parseFixed(str: string): bigint {
  ...
  const frac = (fracRaw + '0'.repeat(Number(DECIMALS))).slice(0, Number(DECIMALS));
...
export function formatFixed(v: bigint): string {
  ...
  const a = neg ? -v : v;
  const int = a / ONE;
  const frac = (a % ONE).toString().padStart(Number(DECIMALS), '0').replace(/0+$/, '');
