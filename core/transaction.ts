...
import { hash256, sign, recoverAddress, fromHex, toHex } from './crypto.js';
import type { Tx, TxType, Program, Hex } from './types.js';
...
function encodeCode(code: Program | null): Uint8Array {
  ...
  const items = code.map((ins) => [ins.op, ins.arg ?? 0n, ins.arg === undefined ? 0n : 1n]);
  return encode(items);
}
...
export function verifyTx(tx: Tx): boolean {
  try {
    ...
    const signer = recoverAddress(digest, fromHex(tx.sig.compact), tx.sig.recovery);
