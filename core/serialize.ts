...
// ---------------------------------------------------------------------------
import type { Encodable } from './types.js';
...
export function encode(value: Encodable): Uint8Array {
  ...
  if (typeof value === 'string') return frame(TAG.STRING, new Uint8Array(Buffer.from(value, 'utf8')));
  if (Array.isArray(value)) return frame(TAG.ARRAY, concat(value.map(encode)));
  throw new Error('serialize: kiểu không hỗ trợ (cấm Number trong dữ liệu đồng thuận)');
}
